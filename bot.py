...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                ...
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. Gabung Group https://t.me/affiliatesidn untuk Bantuan"
                )
                sys.exit()
        if FORCE_SUB_GROUP:
            try:
        ...
        self.LOGGER(__name__).info(
            f"[🔥 BERHASIL DIAKTIFKAN BGST! 🔥]\n\nBOT Dibuat oleh @{OWNER}\nJika @{OWNER} Membutuhkan Bantuan, Silahkan Tanyakan di Grup https://t.me/affiliatesidn"
        )
